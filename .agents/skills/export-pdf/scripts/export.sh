...
validate_pdf() {
  local output="$1"
  if [[ ! -s "$output" ]] || [[ "$(stat -f '%z' "$output")" -lt 10000 ]] || ! rtk proxy file "$output" | grep -q 'PDF document'; then
...
export_one() {
  ...
  case "$source" in
    *.md)
      ...
      rtk proxy "$chrome" --headless --disable-gpu --no-pdf-header-footer --print-to-pdf="$output" "file://$html"
      ;;
    *.html|*.htm)
      rtk proxy "$chrome" --headless --disable-gpu --no-pdf-header-footer --print-to-pdf="$output" "file://$source"
      ;;
    *)
