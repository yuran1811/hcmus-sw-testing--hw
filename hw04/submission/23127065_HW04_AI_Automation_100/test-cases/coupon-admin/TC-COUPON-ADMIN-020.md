# TC-COUPON-ADMIN-020: min_order_amount = 0 (ON, đúng min ≥ 0) → được chấp nhận
...
Coupon Admin / Functional / BVA (B2 ON)
...
- Mã `BVA_MOA_ON` chưa tồn tại trong hệ thống.
...
3. Điền đầy đủ thông tin vào Form "Tạo mã giảm giá mới" theo dữ liệu kiểm thử (giá trị min_order_amount = 0):
   - Mã coupon: `BVA_MOA_ON`
   - Loại: `Phần trăm (%)`
   - Giá trị %: `15`
   - Đơn tối thiểu: `0`
...
- Mã giảm giá `BVA_MOA_ON` được tạo thành công và xuất hiện trong danh sách hiển thị với đơn tối thiểu `0 ₫`.
