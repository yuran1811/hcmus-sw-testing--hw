# TC-COUPON-ADMIN-018: discount_value = 1 (ON, đúng min > 0) → được chấp nhận
...
Coupon Admin / Functional / BVA (B1 ON)
...
- Mã `BVA_DV_ON` chưa tồn tại trong hệ thống.
...
3. Điền đầy đủ thông tin vào Form "Tạo mã giảm giá mới" theo dữ liệu kiểm thử (giá trị discount_value = 1):
   - Mã coupon: `BVA_DV_ON`
   - Loại: `Phần trăm (%)`
   - Giá trị %: `1`
...
- Mã giảm giá `BVA_DV_ON` được tạo thành công và xuất hiện trong danh sách hiển thị với giá trị giảm `1%`.
